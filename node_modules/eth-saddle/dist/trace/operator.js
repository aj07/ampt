...
exports.describeOperation = void 0;
const inversion_1 = require("./inversion");
const utils_1 = require("./utils");
function buildDesc(desc, info) {
    let inputs = '';
    let outputs = '';
    if (desc.inputs.length > 0) {
        const mappedInputs = desc.inputs.map((input) => inversion_1.showInv(info.inv, input));
        inputs = `[${mappedInputs.join(',')}]`;
    }
    if (desc.outputs.length > 0) {
        outputs = `-> 0x${desc.outputs.map(utils_1.trimZero).join(',')}`;
    }
    return {
        inputs: desc.inputs,
...
function buildOpDesc(log, nextStack, info, opInfo) {
    const desc = {
        op: log.op,
        inputs: utils_1.lastN(log.stack, opInfo.inputs),
        outputs: utils_1.lastN(nextStack, opInfo.outputs)
    };
    let inv = info.inv;
    if (opInfo.addInv) {
        inv = inversion_1.addInv(info.inv, log.op, desc.inputs, desc.outputs);
    }
    return {
...
function pushFn(n) {
    return function (log, nextStack, info) {
        return {
            extended: buildDesc({
                op: log.op,
                inputs: utils_1.lastN(nextStack, 1),
                outputs: []
            }, info),
            info: {
                ...info,
                inv: inversion_1.addInv(info.inv, log.op, utils_1.lastN(nextStack, 1), [])
            }
        };
    };
}
function swapFn(n) {
    return function (log, nextStack, info) {
        return {
            extended: buildDesc({
                op: log.op,
                inputs: [utils_1.last(nextStack, 1)],
                outputs: [utils_1.last(nextStack, n)]
            }, info),
            info
        };
    };
...
function dupFn(n) {
    return function (log, nextStack, info) {
        return {
            extended: buildDesc({
                op: log.op,
                inputs: utils_1.lastN(nextStack, n),
                outputs: utils_1.lastN(nextStack, n)
            }, info),
            info
        };
    };
...
function logFn(n) {
    return function (log, nextStack, info) {
        return {
            extended: buildDesc({
                op: log.op,
                inputs: utils_1.lastN(log.stack, n),
                outputs: []
            }, info),
            info
        };
...
function sha(log, nextStack, info) {
    let memory = log.memory || [];
    const memStart = parseInt(utils_1.last(log.stack, 1), 16);
    const memEnd = parseInt(utils_1.last(log.stack, 2), 16);
    let inputs = [memory.join('').slice(memStart * 2, memEnd * 2)];
    let outputs = utils_1.lastN(nextStack, 1);
    let inv = inversion_1.addInv(info.inv, log.op, inputs, outputs);
    return {
...
function describeOperation(log, nextStack, info) {
